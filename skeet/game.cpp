 ...
 **************************************************************************/
Bird* Game :: createBird() //TODO: CREATE OTHER BIRDS
{
   Bird* newBird = NULL;
   Point p;
  
   int randomVal = random(0, 3);
   if (randomVal == 0)
      newBird = new Bird(p);
   else if (randomVal == 1)
...
void Game :: draw(const Interface & ui)
{
   ...
   //draw shoots fired
   Point shootsFired;
   shootsFired.setX(topLeft.getX() + 360);
   shootsFired.setY(topLeft.getY() -5   );
   drawNumber(shootsFired, counter);

   //draw used ammo
   Point usedAmmo;
   usedAmmo.setX(topLeft.getX() + 250);
   usedAmmo.setY(topLeft.getY() - 15);
   drawText(usedAmmo, "Used Ammo");
